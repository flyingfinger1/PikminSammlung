...
GROW = re.compile(r"(\d[\d.,]*)(?:/(\d[\d.,]*))?(?:Schritte|steps)")
...
def parse_seed(run, rec):
    ...
    have = need = None
    if grow_line:
        g = GROW.search(steps_text(grow_line[1]))
        if g and g.group(2):
            have, need = number(g.group(1)), number(g.group(2))
        elif g:
            need = number(g.group(1))
    if need is None:
